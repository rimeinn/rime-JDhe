...
local function get_record_filername(record_type)
	...
	if user_distribute_name:lower():match("ibus") then
		return string.format(
			"%s/rime/lua/jdh/cold_word_records/%s_words.lua",
			os.getenv("HOME") .. "/.config/ibus",
			record_type
		)
	else
		local file_path = string.format("%s/lua/jdh/cold_word_records/%s_words.lua", user_data_dir, record_type)
...
function cold_word_drop.init(env)
	...
	local _sd, drop_words = pcall(require, "jdh/cold_word_records/drop_words")
	local _sh, hide_words = pcall(require, "jdh/cold_word_records/hide_words")
	local _sr, reduce_freq_words = pcall(require, "jdh/cold_word_records/reduce_freq_words")
