local n2cn = require("tools/number_to_cn")
...
function T.func(input, seg, env)
	...
	if seg:has_tag("chinese_lunar") and input:gsub("[%a%/]", ""):match("^[12]%d%d%d%d%d%d%d$") then
		segment.prompt = "〔" .. "农历" .. "〕"
		local solarDateTable = solar2LunarByTime(input:gsub("[%a%/]", ""))
